...
namespace TradeDataMonitorTest
{
    ...
    public class TradeDataMonitorTest
    {
        ...
        {
            // arrange
            var someNewFile = new FileMetadata {FileName = "someNewFile.csv"};
            
            var mockFileSystem = A.Fake<IFileSystem>();
            mockFileSystem.CallsTo(system => system.GetNewFiles(DateTime.MinValue)).Returns(new List<FileMetadata> {someNewFile});
        ...
        public void TradeDataUpdateEventNotFired_NoNewFile_NoException()
        {
            // arrange
            var mockFileSystem = A.Fake<IFileSystem>();
            mockFileSystem.CallsTo(system => system.GetNewFiles(DateTime.MinValue)).Returns(new List<FileMetadata> {});
            var mockTradeDataLoader = A.Fake<ITradeDataLoader>();
